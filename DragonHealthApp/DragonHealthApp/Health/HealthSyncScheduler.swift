...
final class HealthSyncScheduler {
    ...
    static let taskIdentifier = "com.blackdragon.DragonHealthApp.health.refresh"
    ...
    func scheduleNext() {
        ...
        do {
            ...
            if let beginDate = request.earliestBeginDate {
                logger.info("health_sync_scheduled", metadata: ["earliest": "\(beginDate)"])
            } else {
    ...
    private func handleAppRefresh(_ task: BGAppRefreshTask) {
        ...
        let operation = Task.detached {
            await HealthSyncWorker.shared.sync(allowAuthorization: false)
        }
